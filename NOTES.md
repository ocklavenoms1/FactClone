...
- Tests: 42 sub-suites total — 17 soil + 5 fertilizer-chain + 4 fertilizer-applicator + **9 wasteland sub-suites** (trigger + grace + blocks regen + planter idle + recovery + save v18 + composter HIGH recipes + stacking + grace-rescue) + **7 applicator-wasteland-recovery** (audit #4/#5: belt-fed HIGH + HIGH selection + scarred-tile wedge + HIGH restore + accepts-list drift guard both directions + aggregate buffer cap + panel cell colours). The old total here read 31 against addends summing to 35 — corrected while adding the new file; the wasteland list then named only 8 of its 9 ("blocks regen", sub-suite 3, was missing) and the same short list was in `test_wasteland.gd`'s own success string.
