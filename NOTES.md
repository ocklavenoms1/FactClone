...
- Tests: 41 sub-suites total — 17 soil + 5 fertilizer-chain + 4 fertilizer-applicator + **9 wasteland sub-suites** (trigger + grace + recovery + planter idle + save v18 + composter HIGH recipes + stacking + grace-rescue) + **6 applicator-wasteland-recovery** (audit #4/#5: belt-fed HIGH + HIGH selection + scarred-tile wedge + HIGH restore + accepts-list drift guard + aggregate buffer cap). The old total here read 31 against addends summing to 35 — corrected while adding the new file.
